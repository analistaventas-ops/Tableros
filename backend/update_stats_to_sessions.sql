-- SQL Script to update get_enhanced_stats to use activity_sessions and support the new schema
...
DECLARE
    ...
    top_users_data JSON;
BEGIN
    -- 1. Calculate KPIs (Minutes and Unique Users)
    SELECT 
        COALESCE(SUM(duration_minutes), 0),
        COUNT(DISTINCT user_id)
    INTO total_minutes, unique_users
    FROM activity_sessions s
    WHERE (start_date IS NULL OR s.session_date >= start_date)
      AND (end_date IS NULL OR s.session_date <= end_date)
    ...
    -- 2. Over Time Data (Session counts per day)
    ...
    -- 3. By Dashboard Data (Top Dashboards by use)
    SELECT json_agg(t) INTO by_dashboard_data
    FROM (
        SELECT 
            COALESCE(dt.name, 'Otro/General') as name,
            COUNT(*) as count
        FROM activity_sessions s
